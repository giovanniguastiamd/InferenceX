    ...
    if [[ "${RUN_EVAL}" == "true" ]]; then
        ...
        if [ -n "$EVAL_DIR" ] && [ -d "$EVAL_DIR" ]; then
            ...
            for eval_file in "$EVAL_DIR"/*; do
                ...
                # stale root-owned files left by prior runs. -n (non-interactive)
                # is a fork-local change: this runner has no passwordless sudo and
                # would otherwise block on a password prompt.
    ...
    if [[ "${IS_AGENTIC}" == "1" ]]; then
        ...
        if [ -d "$JOB_LOGS_DIR" ]; then
            ...
            if [ -d "$AGENTIC_SRC" ] && find "$AGENTIC_SRC" -mindepth 1 -maxdepth 1 -type d -name 'conc_*' -print -quit 2>/dev/null | grep -q .; then
                ...
                # Fork-local: chown is dropped (no passwordless sudo here, and
                # NFS root_squash maps container-root to nobody anyway); the
                # chmod below is enough to make LOGS/ removable.
